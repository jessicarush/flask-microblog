...
class User(UserMixin, db.Model):
    ...
    def add_notification(self, name, data):
        ...
        db.session.add(n)
        return n
        # This method not only adds a notification for the user to the
        # database, but also ensures that if a notification with the same name
        # already exists, it's removed first. The notification name will be:
        # unread_message_count. If the database already has a notification with
        # this name with, for example, a value of 3, whenever the user
        # receives a new message and the message count goes to 4, we want to
...
class Task(db.Model):
    # An interesting difference between this model and the previous ones is
    # that the id primary key field is a string, not an integer. This is
    # because for this model, we're going to use the job identifiers generated
    # by RQ instead of the database auto-generated keys.
